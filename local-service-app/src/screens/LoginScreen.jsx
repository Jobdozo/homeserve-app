...
export default function LoginScreen() {
  ...
  async function handleRequestOtp(e) {
    ...
    setBusy(true);
    try {
      const res = await api.requestOtp(phone.trim(), "customer");
  ...
  async function handleVerifyOtp(e) {
    ...
    setBusy(true);
    try {
      const { token, user } = await api.verifyOtp(phone.trim(), code.trim(), "customer", name.trim());
      login(token, user);
    } catch (err) {
        ...
        <div className="phone-frame">
          <div className="screen no-scrollbar flex flex-col justify-center px-6 py-10">
            <div className="mb-8 flex flex-col items-center gap-2 text-center">
              <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand text-2xl text-white">
                🧑‍🔧
              </span>
              <h1 className="text-xl font-extrabold text-gray-900">Tikdum</h1>
              <p className="text-[13px] text-gray-400">Log in or sign up with WhatsApp</p>
            </div>
            ...
            {step === "phone" && (
              <form onSubmit={handleRequestOtp} className="flex flex-col gap-3">
                ...
                <label className="text-[13px] font-semibold text-gray-700">
                  Name <span className="font-normal text-gray-400">(new accounts only)</span>
                  <input
                    type="text"
                    value={name}
